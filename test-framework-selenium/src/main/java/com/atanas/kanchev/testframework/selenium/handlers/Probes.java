...
public final class Probes implements IProbe, IProbeExpectedConditions {
    ...
    @Override public boolean hasAnyText() {
        ...
        boolean hasAnyText =
            !context().<SeleniumContext>getCurrentContext().getCurrentElement().getText().isEmpty();
        ...
        switch (css) {
            case CSS_BACKGROUND_COLOUR:
                Color backgroundColor = Color.fromString(
                    context().<SeleniumContext>getCurrentContext().getCurrentElement().getCssValue(
    ...
    @Override public boolean hasLinkToURL(String url) {
        ...
        context().<SeleniumContext>getCurrentContext().setWebElementsList(
            context().<SeleniumContext<WebDriver>>getCurrentContext().getDriver()
    ...
    @Override public boolean followLinkToURL(String link) {
        ...
        if (hasLinkToURL(link)) {
            context().<SeleniumContext<WebDriver>>getCurrentContext().getDriver().navigate()
                .to(link);
            canFollow = true;
        }
        return canFollow;
    ...
    @Override public boolean hasPartialCookieValue(String cookieName, String cookieValue) {
        ...
        try {
            ...
            if (context().<SeleniumContext<WebDriver>>getCurrentContext().getDriver().manage()
            ...
            for (Cookie cookie : context().<SeleniumContext<WebDriver>>getCurrentContext()
        ...
        logger.error("Cookie does not contain the value: " + cookieValue + ", actual value: "
            + (context().<SeleniumContext<WebDriver>>getCurrentContext().getDriver().manage()
    ...
    @Override public boolean titleIs(String title) {
        ...
        return ExpectedConditions.titleIs(title)
            .apply(context().<SeleniumContext<WebDriver>>getCurrentContext().getDriver());
    }
